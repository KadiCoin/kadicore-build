 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`kadicore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`kadicore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'kadicore-' + name : 'kadicore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/kadicore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external kadicore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:kadicore-lib -o kadicore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('KadiCoin-project', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('KadiCoin-project', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push KadiCoin-project $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
